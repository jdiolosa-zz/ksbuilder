...
#mendarie   sept 12 2017  1.0
#mendarie   sept 19 2017  2.0 added Disk, User, groups
#
## ----------------------------------
...
function get_city () {
local choice
	read -p "Enter City [ ny or ca ] " choice
	case $choice in
		ny) city=ny ;;
		ca) city=ca ;;
...
function get_datavg_add () {
	echo -n "Enter Mountpoint location: "
	read datavg_dir_loc
...
function get_user_add () {
	...
	run_added_users+=("/bin/sed -i \"/^#Users.*$/auseradd -u $user_gid -g \'$user_group_name\' -d \'$user_home_dir2\' -m -p \'$user_passwd\' -s \'$user_shell\' -c \'$user_gecos\' $user_name"\")
}
...
function Write_KS () {
	...
	if [[ $city = "ny" ]]; then
		svn export http://hostname.com/kickstart/ny-RHEL7.cfg /var/www/html/pub/$name.cfg
		
	elif [[ $city = "stl" ]]; then
		svn export http://hostname/linux/kickstart/ca-RHEL7.cfg /var/www/html/pub/$name.cfg
	fi
